...
public sealed partial class CharacterRecordViewer : FancyWindow
{
    ...
    private FullCharacterRecords? _currentRecord;
    ...
    public CharacterRecordViewer(EntityUid console)
    {
        ...
        _filterType = StationRecordFilterType.Name;
        ...
        RecordEntryViewType.AddItem(Loc.GetString("department-Security"), (int)RecordConsoleType.Security);
        ...
        RecordEntryViewType.OnItemSelected += args =>
        {
            ...
            if (_currentRecord != null)
                UpdateAdminEntryList(_currentRecord);
        };
    }
    ...
    private void UpdateAdminEntryList(FullCharacterRecords record)
    {
        ...
        {
            case RecordConsoleType.Employment:
                SetEntries(record.PRecords.EmploymentEntries, true);
                break;
            case RecordConsoleType.Medical:
                SetEntries(record.PRecords.MedicalEntries, true);
                break;
            case RecordConsoleType.Security:
    ...
    private void UpdateRecordBoxSecurity(FullCharacterRecords record, CriminalRecord? criminal)
    {
        ...
        var status = criminal?.Status ?? SecurityStatus.None;
        PersonStatus.Text = Loc.GetString($"criminal-records-status-{status.ToString().ToLower()}");
        ...
        if (criminal != null)
        {
            ...
            StatusOptionButton.SelectId((int)status);
            ...
            RecordContainerCriminalHistoryButton.Disabled = false;
        }
        else
        {
            _selectedCriminalRecord = null;
            StatusOptionButton.SelectId((int)SecurityStatus.None);
            RecordContainerWantedReason.SetMessage(string.Empty);
            RecordContainerWantedReasonContainer.Visible = false;
            // Close any open history windows.
            OnHistoryClosed?.Invoke();
            RecordContainerCriminalHistoryButton.Disabled = true;
        }

        RecordContainerWantedIcon.SetFromSpriteSpecifier(GetWantedStatusIcon(status));
    ...
    private SpriteSpecifier GetWantedStatusIcon(SecurityStatus status)
    {
        ...
        return new SpriteSpecifier.Rsi(new ResPath("Interface/Misc/security_icons.rsi"), state);
    }
